    ...
    public function behaviors()
    {
       return [
         'access' => [
             'rules' => [
                 'action' => ['listStock'],
                 'roles' => [UserOperation::RoleAdmin, UserOperation::RoleUser],
                 'matchCallback' => function (){
                        $this->redirect('/user/login');
                 }
             ],
             [
                 'action' => ['addStock', 'editStock', 'deleteStock'],
                 'roles' => [UserOperation::RoleSeller],
                 'matchCallback' => function (){
                        $this->redirect('/magazineBlock/magazine');
                 }
             ]
         ]
       ];
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['btnAddStock'])){
            ...
            if (!empty($stock)){
                ...
                if ($result_add['result']){
                    $this->redirect('/magazineBlock/listStock');
                } else{
